...
import { Banknote, Calendar, Clock, FileText, MapPin, PackageCheck, Phone, ShoppingBag, User, Wind, Thermometer, AlertTriangle, Truck, Store, Ticket, Percent, CreditCard } from 'lucide-react';
...
const mapMagentoCartItems = (items: MagentoCartItem[] = []): CheckoutItem[] => {
  ...
  return items.map((item) => {
    ...
    const originalPrice = matchingProduct
      ? toFiniteMoney(rawPrice)
      : toFiniteMoney(item.prices?.price?.value ?? product.price_range?.minimum_price?.final_price?.value ?? 0);
...
export function CheckoutPage() {
  ...
  const totalAmount = useMemo(() => {
    return toFiniteMoney(subtotal) - toFiniteMoney(voucherDiscount) + toFiniteMoney(shippingFee) - toFiniteMoney(shippingDiscount) + toFiniteMoney(weatherSurcharge);

...
const applyWholesaleDiscountToLocalItem = (item: CartLineItem, newQuantity: number): CartLineItem => {
  ...
  const specialPrice = toFiniteCurrencyNumber(parsePrice(matchingProduct.special_price ?? matchingProduct.specialPrice));
  const normalPrice = toFiniteCurrencyNumber(parsePrice(matchingProduct.price ?? matchingProduct.priceValue));
  const originalPrice = specialPrice > 0 ? specialPrice : normalPrice;
